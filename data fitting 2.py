...
# 두 데이터 세트를 함께 피팅하기 위한 모델 함수 (분리 인덱스를 인수로 받도록 수정)
def combined_radial_velocity_model(t, split_index, gamma, K1, P, e, omega1, t01, K2, omega2, t02):
    # 첫 번째 데이터 세트의 인덱스를 사용하여 데이터 분리
    t1 = t[:split_index]
    t2 = t[split_index:]
...
def fit_and_plot(t_data1, vr_observed1, t_data2, vr_observed2):
    ...
    # 초기 추정값 설정을 위해 데이터 세트가 비어있지 않은지 확인
    K1_guess = (np.max(vr_observed1) - np.min(vr_observed1)) / 2 if vr_observed1.size > 0 else 10
    K2_guess = (np.max(vr_observed2) - np.min(vr_observed2)) / 2 if vr_observed2.size > 0 else 10
    ...
    # curve_fit에 전달할 래퍼 함수 정의 (split_index를 고정)
    def model_for_fit(t, gamma, K1, P, e, omega1, t01, K2, omega2, t02):
        return combined_radial_velocity_model(t, len(t_data1), gamma, K1, P, e, omega1, t01, K2, omega2, t02)
    
    popt, pcov = curve_fit(
        model_for_fit, t_combined, vr_combined,
    ...
    t_model2 = np.linspace(min(t_data2), max(t_data2), 500) if t_data2.size > 0 else np.array([])
    
    # 각 데이터 세트에 대해 모델 곡선 계산
    t_model_combined = np.concatenate((t_model1, t_model2))
    vr_model_combined = combined_radial_velocity_model(t_model_combined, len(t_model1), *popt)
    
    vr_model1 = vr_model_combined[:len(t_model1)]
    vr_model2 = vr_model_combined[len(t_model1):]
    
    # 잔차 계산은 원래 데이터 포인트 기준으로
    residuals1 = vr_observed1 - combined_radial_velocity_model(np.concatenate((t_data1, t_data2)), len(t_data1), *popt)[:len(t_data1)]
    residuals2 = vr_observed2 - combined_radial_velocity_model(np.concatenate((t_data1, t_data2)), len(t_data1), *popt)[len(t_data1):]
...
if __name__ == '__main__':
    ...
    try:
        t_data1 = np.array([float(x) for x in t_data1_input.split(",") if x.strip()])
        vr_observed1 = np.array([float(x) for x in vr_observed1_input.split(",") if x.strip()])
        t_data2 = np.array([float(x) for x in t_data2_input.split(",") if x.strip()])
        vr_observed2 = np.array([float(x) for x in vr_observed2_input.split(",") if x.strip()])
    except Exception:
        st.error("입력 데이터는 콤마로 구분된 숫자여야 합니다.")
        st.stop()
        
    if st.button("피팅 실행"):
        # 두 데이터 세트 중 하나라도 비어 있으면 오류 메시지 표시
        if t_data1.size == 0 or vr_observed1.size == 0 or t_data2.size == 0 or vr_observed2.size == 0:
            st.error("두 데이터 세트의 시간과 시선속도 데이터를 모두 입력해야 합니다.")
        else:
